...
(async () => {
  ...
  app.post('/schedules', async (req, res) => {
    try {
      ...
      await db.run('INSERT INTO schedules (shareId, data, version, updatedAt) VALUES (?, ?, ?, ?)',
        shareId, data, 1, now);
  ...
  app.get('/schedules/:shareId', async (req, res) => {
    try {
      const row = await db.get('SELECT * FROM schedules WHERE shareId = ?', req.params.shareId);
      if (!row) return res.status(404).json({ error: 'not found' });
      res.json({ shareId: row.shareId, schedule: JSON.parse(row.data), version: row.version, updatedAt: row.updatedAt });
    } catch (err) {
  ...
  // Update schedule (optimistic version check)
  app.put('/schedules/:shareId', async (req, res) => {
    try {
      ...
      const row = await db.get('SELECT version FROM schedules WHERE shareId = ?', shareId);
      if (!row) return res.status(404).json({ error: 'not found' });
      ...
      const now = new Date().toISOString();
      await db.run('UPDATE schedules SET data = ?, version = ?, updatedAt = ? WHERE shareId = ?',
